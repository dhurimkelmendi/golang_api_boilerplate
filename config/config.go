...
type Config struct {
	...
	// APIHost is the host (with protocol) to the  API without trailing slash, eg: https://staging..com
	APIHost string

	// RespondWithInnerError determines if API error response should include inner error messages.
	RespondWithInnerError bool

	// SampleStoryTitle is the title of the sample story that users will be able to acess once they are registered/created.
	SampleStoryTitle string

	// AvatarBucketURL is the URL of the avatar S3 bucket
...
func (c *Config) readConfigs() {
	...
	c.DatabaseName = appConfig.GetConfig("DB_NAME", "project_name_db")
	c.DatabaseUsername = appConfig.GetConfig("DB_USERNAME", "project_name_backend")
	c.DatabasePassword = appConfig.GetConfig("DB_PASSWORD", "project_name_password")
	c.HTTPAddr = appConfig.GetConfig("HTTP_ADDR", ":8080")
	c.JWTSecret = appConfig.GetConfig("JWT_SECRET", "jwt_secret_signing_key")
	c.APISecret = appConfig.GetConfig("_API_SECRET", "_app_secret_signing_key")
	c.APIHost = appConfig.GetConfig("_API_HOST", "http://localhost:8090")
	c.SampleStoryTitle = appConfig.GetConfig("SAMPLE_STORY_TITLE", "My First Story")
	c.AvatarBucketURL = appConfig.GetConfig("AVATAR_BUCKET_URL", "https://-adventures-staging.s3.us-west-2.amazonaws.com/avatars")
...
func (c *Config) LogConfigs() {
	...
	logrus.Warn(fmt.Sprintf("  * SampleStoryTitle: %+v", c.SampleStoryTitle))
	logrus.Warn(fmt.Sprintf("  * AvatarBucketURL: %+v", c.AvatarBucketURL))
}
